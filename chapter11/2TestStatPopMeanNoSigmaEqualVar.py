...
tDenominator = math.sqrt((((n1-1)*(s1**2))+((n2-1)*(s2**2)))/(n1+n2-2)) * math.sqrt((1/n1)+(1/n2))
t = tNumerator/tDenominator
print()
print("T is: " + str(t) + ".")
print()
cl = float(input("What is the confidence level? "))
if cl >= 1:
    cl = cl/100
    cl = 1 - cl
    cl = cl/2
...
df = n1 + n2 - 2
if hypo == "=":
    tAlpha = abs(scipy.stats.t.ppf(cl/2, df))
    if abs(t) >= tAlpha:
        state = True
elif hypo =="<":
    tAlpha = -abs(scipy.stats.t.ppf(cl, df))
    if t <= tAlpha:
        state = True
elif hypo ==">":
    tAlpha = abs(scipy.stats.t.ppf(cl, df))
